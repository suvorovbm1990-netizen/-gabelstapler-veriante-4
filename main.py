...
QUESTIONS = [
    ...
    {"id": 2, "frage": "Eine Kiste mit 1600 kg soll gestapelt werden. Unter welchen Bedingungen ist dies möglich? (Siehe Diagramm)", "image": "images/2.jpg", "options": ['Lastschwerpunkt 600 mm und Höhe 4,5 m;', 'Lastschwerpunkt 600 mm und Höhe 5,5 m;', 'Lastschwerpunkt 700 mm und Höhe 4,0 m;', 'Lastschwerpunkt 500 mm und Höhe 5,0 m.'], "correct": [0, 2]},
    ...
    {"id": 4, "frage": "Wann ist die nächste Prüfung bei dieser Prüfplakette fällig? (Plakette zeigt 06 mit Pfeil auf 2)", "image": "images/4.jpg", "options": ['Februar 2006;', 'August 2006;', 'Februar 2007;', 'August 2007.'], "correct": [0]},
    ...
    {"id": 12, "frage": "Was ist beim Befahren von Ladebrücken zu beachten?", "image": "images/12.jpg", "options": ['Nichts;', 'Nur rückwärts;', 'Tragfähigkeit der Ladebrücke;', 'Tragfähigkeit der Ladefläche;', 'Nach UVV verboten.'], "correct": [2, 3]},
    {"id": 13, "frage": "Welche Bezeichnung ist für das Gebotszeichen blau/weiss richtig?", "image": "images/13.jpg", "options": ['Fussweg benutzen;', 'Heizraum für Schuhe;', 'Festes Schuhwerk verboten;', 'Schutzschuhe tragen.'], "correct": [3]},
    {"id": 14, "frage": "Auf welche Höhe dürfen Sie Palette 800x1200 mm, 1100 kg heben? (Traglast 1500kg Diagramm)", "image": "images/14.jpg", "options": ['4,0 m;', '4,5 m;', '5,0 m;', '5,5 m;', '3,5 m.'], "correct": [2]},
    ...
    {"id": 18, "frage": "Auf welcher Darstellung ist der Lastschwerpunktabstand am größten?", "image": "images/18.jpg", "options": ['Darstellung a - Last weit vorn', 'Darstellung b - Last nah', 'Darstellung c - Last direkt am Rücken'], "correct": [0]},
...
if current_idx >= total:
    ...
    if prozent == 100:
        st.balloons()
        st.success("Perfekt! 100% - Du bist bereit für die Prüfung!")
    elif prozent >= 80:
        st.success(f"{prozent:.0f}% - Sehr gut!")
    else:
        st.warning(f"{prozent:.0f}% - Noch mal üben!")
    ...
    if q["image"]:
        if os.path.exists(q["image"]):
            ...
            st.info(f"📷 Bild fehlt: {q['image']} - lege es in den Ordner images/ auf GitHub")
        ...
        if not st.session_state.answered:
            if st.button("✅ Antworten", type="primary", use_container_width=True):
                if not user_selection:
                    st.warning("Bitte wähle mindestens eine Antwort!")
                else:
